...
if [ ${#FAILED_DAQS[@]} -eq 0 ]; then
    ...
    echo ""
    echo "=========================================="
    echo "Stage 4: Combined Multi-Sensor Analysis"
    echo "=========================================="

    COMBINED_OUTPUT_DIR="${BASE_DATA_DIR}/combined"
    mkdir -p "$COMBINED_OUTPUT_DIR"
    mkdir -p "${COMBINED_OUTPUT_DIR}/plots"

    echo "Running combined analysis..."
    "${SCRIPT_DIR}/combined_analysis" \
        --daq01-root "${BASE_DATA_DIR}/daq01/output/root/waveforms_analyzed.root" \
        --daq02-root "${BASE_DATA_DIR}/daq02/output/root/waveforms_analyzed.root" \
        --output-dir "$COMBINED_OUTPUT_DIR" \
        --verbose

    if [ $? -eq 0 ]; then
        echo ""
        echo "SUCCESS: Combined analysis completed"
        echo "  ROOT file: ${COMBINED_OUTPUT_DIR}/combined_analysis.root"
        echo "  Summary: ${COMBINED_OUTPUT_DIR}/analysis_summary.txt"
        echo "  Plots: ${COMBINED_OUTPUT_DIR}/plots/"
        echo ""
    else
        echo ""
